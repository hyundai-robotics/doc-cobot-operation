...
* **직접 교시**: 직접교시 기능의 사용여부를 결정합니다.
* **직접 교시 상시 활성화**: 모터온 시 바로 직접교시로 동작합니다.
* **구속 모션**: '직접 교시 상시 활성화'나 '콕핏 M 버튼 '가 직접교시 모드에서 버튼 클릭시 실행되는 모드를 결정합니다.  
* **콕핏 설정**: Cockpit버튼의 모드를 결정합니다. 각 '콕핏 A, B, C 버튼'은 아래의 보기중 하나의 기능을 가질 수 있습니다.
...
* **축 잠금**:조인트 모션에서 특정 축의 가동을 막을 수 있음 - 활성화시 축 구동중
