...
{% hint style="warning" %}
Les informations présentées dans ce manuel sont la propriété de Hyundai Robotics.

Le manuel ne peut être copié, en partie ou en totalité, ni redistribué sans le consentement écrit préalable de Hyundai Robotics.

Il ne peut être fourni à des tiers ni utilisé à d'autres fins.

Hyundai Robotics se réserve le droit de modifier ce document sans notification préalable.

**Copyright ⓒ 2020 par Hyundai Robotics**
{% endhint %}
...
* **Direct teaching On**: Upon motor activation, the system immediately operates in direct teaching mode. The motor remains active—and direct teaching enabled—only while the enable switch is pressed. To switch the robot back to Jog or Automatic mode, you must disable the "Direct Teaching On" setting.
...
- The image above illustrates the cockpit buttons attached to the HDC series of collaborative robots. Please refer to this image when reading the cockpit settings below.
...
* **Active Mode** : You can check the currently active mode. The display format is the same as that of the 'Cockpit Settings'.
...
* When performing direct teaching, you can proceed in the same way as you would with standard jogging.
...
* Vous devez définir une nouvelle position de mastering dans les cas suivants. L'utilisation de la position de mastering précédemment enregistrée peut entraîner un mastering incorrect ou des accidents.
    ...
    - Initialisation du codeur dans le `[F2: system] - 3: Robot parameter - 4: Encoder Offset` menu
{% endhint %}
