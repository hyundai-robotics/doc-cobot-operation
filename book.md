...
* **직접 교시 활성화 (Direct teaching On)**: 모터온 시 바로 직접교시로 동작합니다. Enable Switch가 눌러져 있어야만 모터온이며, 직접교시가 구동됩니다. 다시 조그나 자동으로 로봇을 원할 경우 설정내 '직접 교시 활성화 (Direct teaching On)'를 OFF 해야함 
...
- 위 사진은 협동로봇 시리즈 HDC에 부착된 콕핏 버튼을 형상황한 그림입니다. 참고하여 아래 콕핏 설정을 읽어주시기 바랍니다.
...
* **Active Mode** : 현재 실행중인 모드를 확인할 수 있습니다. 표시 형식은 '콕핏 설정'과 동일합니다.
...
* 직접교시를 실행하고 할때 기존 조그와 동일한 방식으로 실행하시면 됩니다.
* 모터온 후 Enable Switch를 누르고 계셔야만 직접교시가 계속 실행됩니다.
* 직접교시 실행 중 '직접 교시 활성화(Direct teaching On)'를 OFF로 변경하시거나 Enable Switch에서 손을 때시면 직접교시가 정지 됩니다.
...
* 아래의 경우, 마스터링 위치를 새롭게 설정해야 합니다. 기존에 등록된 마스터링 위치를 그대로 사용할 경우, 정상적으로 마스터링이 되지 않거나 사고가 발생할 수 있습니다.
    - 2차 엔코더 교체
    -  `[F2: 시스템] - 3: 로봇 파라미터 - 4: 엔코더 옵셋` 메뉴에서 엔코더 초기화 (전체 초기화 버튼)
{% endhint %}
